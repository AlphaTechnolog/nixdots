-- huge thanks to tokyonight.nvim: https://github.com/folke/tokyonight.nvim/blob/main/lua/tokyonight/treesitter.lua
local utbl = require "decay.utils.table"

local ts = { mt = {}, _private = {} }
local setmetatable = setmetatable

function ts:new_style()
  return vim.treesitter.highlighter.hl_map == nil
end

function ts:get(group)
  if group:sub(1, 1) ~= "@" or self:new_style() then
    return group
  end
  ...
  local lang
  while group do
    if self.fallbacks[group] then
      return (lang or "") .. self.fallbacks[group]
    end
    group, lang = group:match("(.*)%.(.*)")
  end
end

function ts:setup_highlights()
  ...
  -- taken from https://github.com/folke/tokyonight.nvim/blob/f247ee700b569ed43f39320413a13ba9b0aef0db/lua/tokyonight/treesitter.lua#L100
  self.defaults = {
    ["@annotation"] = {
      default = true,
      link = "PreProc",
...
local function new()
  local ret = utbl({}):merge(ts)
  ret:setup_highlights()
  return ret
end

function ts.mt:__call()
  return new()
end

return setmetatable(ts, ts.mt)
